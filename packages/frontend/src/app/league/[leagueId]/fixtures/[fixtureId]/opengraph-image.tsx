...
export default async function Image({
  ...
  const fixture = await safeJson<FixtureDetail>(`${API_BASE}/api/fixtures/${params.fixtureId}`);

  const homeTeamId = fixture?.homeTeam.id;
  const awayTeamId = fixture?.awayTeam.id;

  const [homeImpact, awayImpact] = await Promise.all([
    homeTeamId ? safeJson<InjuryImpact>(`${API_BASE}/api/analysis/injury-impact/${homeTeamId}`) : null,
    awayTeamId ? safeJson<InjuryImpact>(`${API_BASE}/api/analysis/injury-impact/${awayTeamId}`) : null,
  ]);
    ...
    (
      ...
      >
        ...
        >
          ...
          >
            <div style={{ fontSize: 52, fontWeight: 300, color: '#f4f4f4', lineHeight: 1.1, marginBottom: 24 }}>
              {homeName}
            </div>
            {homeOut !== null && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#fa4d56', display: 'flex' }} />
                  <span style={{ color: '#c6c6c6', fontSize: 20 }}>{homeOut} out</span>
                </div>
                {homePL !== null && (
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    <span style={{ fontSize: 32, fontWeight: 300, color: powerColor(homePL) }}>{homePL}%</span>
                    <span style={{ color: '#8d8d8d', fontSize: 16 }}>power loss</span>
                  </div>
                )}
              </div>
            )}
          ...
          >
            <div style={{ fontSize: 28, color: '#393939', fontWeight: 300, letterSpacing: '0.1em' }}>VS</div>
          </div>
          ...
          >
            <div style={{ fontSize: 52, fontWeight: 300, color: '#f4f4f4', lineHeight: 1.1, marginBottom: 24, textAlign: 'right' }}>
              {awayName}
            </div>
            {awayOut !== null && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#fa4d56', display: 'flex' }} />
                  <span style={{ color: '#c6c6c6', fontSize: 20 }}>{awayOut} out</span>
                </div>
                {awayPL !== null && (
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    <span style={{ fontSize: 32, fontWeight: 300, color: powerColor(awayPL) }}>{awayPL}%</span>
                    <span style={{ color: '#8d8d8d', fontSize: 16 }}>power loss</span>
                  </div>
                )}
              </div>
        ...
        >
          Predicted lineups · Recovery signals · squadcheck.vercel.app
        </div>
      </div>
    ),
    { ...size },
